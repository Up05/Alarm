...
namespace server {
	...
	u_short server_port = 348; // Port that the server LISTENS to!
	uint32_t client_port = 347;
...
#define CLOSE_SERVER(MESSAGE, CODE)			\
	...
	void init() {
		...
		server_port = io::get_server_port();
		client_port = io::get_client_port();
		...
		InetPton(AF_INET, "127.0.0.1", &client.sin_addr.s_addr);
		...
		LOG2("server", "Started server on port: %u", server_port);
